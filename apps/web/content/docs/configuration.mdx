...
| `relay.controlBoxUrl` | string | empty | URL of a deployed [control plane](/docs/control-plane); cloud boxes point at it for git-token leasing, permission state, and the registry/events so they keep working with the laptop off. Set via `agentbox control-plane set-url` |
