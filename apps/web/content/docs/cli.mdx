...
The agent-agnostic `agentbox attach [box]` reattaches to whichever agent session is running in the box (it prompts when more than one is live). It never auto-starts: when nothing is running it prints a warning and exits non-zero. Each agent command also exposes its own `attach` / `start` subcommands: per-agent `attach` reattaches *and* starts a session if none is running (never re-syncs config), `start` (re)starts a session in an existing box (rsyncs host config unless `--no-sync-config`). `claude`, `codex`, and `opencode` also have a `login` subcommand for their respective auth flows; **pi has no `login` subcommand** — authenticate via API-key env vars or `~/.pi/agent/auth.json` (see [run an agent](/docs/run-an-agent)).
...
agentbox pi -- --model zai-glm/glm-5.2
```
...
Agent-specific: `--attach-in <split|window|tab|same>`, `-d, --no-attach`, `-i, --initial-prompt <text>` (background queue), and for `claude` only `-c, --continue`, `--resume <id>`, `--plan <path>`, and `--dangerously-skip-permissions` (on by default in boxes). `fork` adds `--agent`, `--session <id>`, and `--attach-in` (default `tab`). pi adds `--isolate-pi-config` (per-box volume instead of the shared `agentbox-pi-config`); pi does **not** support `-c`/`--resume` (session teleport) or a `login` subcommand.

<Callout type="warn" title="HEADS UP">`agentbox pi` runs on **Docker and Daytona** (`--provider daytona`, requires `agentbox prepare --provider daytona`). Hetzner, Vercel, and E2B are not yet supported for pi.</Callout>
...
`download` pulls `/workspace` back into your host workspace, honouring `.gitignore`. It has typed subcommands for agent state: `download env`, `download claude`, `download codex`, `download opencode`, `download pi`, `download config`. `cp` copies files or directories in either direction, like `docker cp`. `git` runs git/gh operations against the box from the host through the relay, so push and fetch use your host credentials and the box never sees them.
