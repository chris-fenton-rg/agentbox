...
| `ntn page create …` | write | **Prompts** the host for approval; on `y` the host runs `ntn page create` and ships back the result. |
| `ntn page update …` | write | **Prompts** the host for approval; covers props and archive. |
...
ntn page create -p <db-id> -t "Draft from the box" # prompts on the host
```
