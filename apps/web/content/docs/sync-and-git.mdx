...
<Callout type="warn" title="HEADS UP">
The box never holds your SSH keys or tokens — but plain `git push` inside the box still works. A `git` (and `gh`) shim on the box's `PATH` transparently routes the network ops through the host relay, which runs them on your machine with your credentials and prompts you to approve writes; only the output streams back, so you never type `agentbox-ctl` yourself. One constraint: let the shim use the box's own branch — run a bare `git push`, not `git push <remote> <branch>` (an explicit refspec is rejected).
</Callout>
